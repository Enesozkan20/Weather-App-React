import { useState } from "react";
import getWeatherData from "./api"

const Weather = () => {
    // State olustur
    const [isShow, setIsShow] = useState(false);

    const [name, setName] = useState()
    const [temp, setTemp] = useState()
    const [feelTemp, setFeelTemp] = useState()
    const [maxTemp, setMaxTemp] = useState()
    const [minTemp, setMinTemp] = useState()

    const handleSubmit = async (e) => {
        // Sayfa yenilemesini engelle
        e.preventDefault();

        //Inputtaki sehir adina eris
        const cityName = e.target[0].value;

        //Sehir adina göre api istegi at
        const weatherData = await getWeatherData(cityName);

        // Gelen veri içerisindeki değerleri state'lere aktar
        setName(weatherData.name);
        setTemp(weatherData.main.temp);
        setFeelTemp(weatherData.main.feels_like);
        setMaxTemp(weatherData.main.temp_max);
        setMinTemp(weatherData.main.temp_min);

        // Sonuclari göster
        setIsShow(true);

        //Input alanini temizle
        e.target.reset();
    };
    console.log(name);
    console.log(temp);
    console.log(feelTemp);
    console.log(maxTemp);
    console.log(minTemp);

    return (
        <div>
            <h1>Hava Durumu Uygulamasi</h1>

            <form onSubmit={handleSubmit}>
                <input type="search" placeholder="Sehir adi giriniz..." />
                <div>
                    <button>Ara</button>
                </div>
            </form>

            {
                isShow &&
                <div className="results">
                    <h3 className="city">{name}</h3>
                    <div className="info">
                        <p className="degree">Hava Sicakligi: {temp} ℃</p>
                        <p className="fell-degree"><span>Hissedilen Sicaklik: </span>{feelTemp} ℃</p>
                        <p className="max-degree"><span>Max Sicaklik: </span>{maxTemp} ℃</p>
                        <p className="min-degree"><span>Min Sicaklik: </span>{minTemp} ℃</p>
                    </div>
                </div>
            }

        </div>
    )
}

export default Weather
